...
func (nd *newsDao) FindOne(id string)  (News,error) {
	...
	if(err != nil){
		nd.logger.Fatal("NewsDao FindId error ", err,id)
		return news,err
	}
	return news,nil
}
...
func (nd *newsDao) Save(news News) (News,error){
	...
	if(news.Id.Hex() == ""){
		...
		if(err != nil){
			nd.logger.Fatalln("insert news error,", err, news)
			return news,err
		}
	}else{
		// 获取更新前的数据
		...
		if(err != nil){
			nd.logger.Fatalln("update news error", err, news)
			return news,err
		}
		// 不能从界面转递的数据从表中获取
...
func (nd *newsDao) FindByTypeId(id string)([]News,error){
	...
	if(err != nil){
		log.Fatalln("NewsDao findByTypeId error", id, err)
	}
	return news,err
}
